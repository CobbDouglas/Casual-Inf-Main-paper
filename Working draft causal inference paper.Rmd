...
In 2023, it had been estimated that at least 58.7 million adults in the United States had been diagnosed with some sort of mental illness @samsha2023. In the same year, 14.6 million adults were estimated to have a mental illness that resulted in acute functional impairment or interfered with at least one or more major daily life activities @samsha2023. Of those 14.6 million adults, only 66.7% received any mental health treatment in the past year @samsha2023. According to the Centers for Disease Control's (CDC) WISQARS Leading Causes of Death Report, suicides are the second leading cause of death amongst people aged 10-34 and are ranked as the 11th overall leading cause of death. Within two decades, suicide rates have gradually increased from 10.5 suicides per 100,000 people in the year 2000 to 14.2 per 100,000 in 2018 @centers. Suicide rates vary from state to state, with both East and West Coast supporting low rates such as 7.4 per 100,000 people while mid-western states suffer from rates as high as 25 per 100,000 people @centers. Modern economists use two-way fixed effects to control for these differences between states. Recently in the econometrics literature, more attention has been given to the problem of dynamic timing of federal policies and the potential bias it might bring to traditional two-way fixed effects. This study attempts to replicate @lang2013 findings of the effects of the Mental Health Parity Act of 1996 using @sun2021, @callaway2021, and @goodman-bacon2021 as a way to estimate the average treatment effect while deconstructing the two-way fixed effect regression. Additionally, several propensity score methods are used to compare with replicated and new results. My research finds that traditional methods are in line with previous results while dynamic timing methods estimate somewhere at 3% to 5% depending on the method and base period used. [ @klick2006 ; @centers]

### Lang (2013)

@lang2013 attempts to identify causal effects using difference-in-difference methods and fixed effects using policy shocks which were the result from the after-effects of the Mental Health Parity Act of 1996 (MHPA). @lang2013 showed a statistically significant effect of a 4-7% decrease in suicide rate after policy implementation using two-way fixed effects. To do this with the rollout of policies from the states, @lang2013 set 1998 as $t_0$ since most of the state-level policies were enacted afterward. This paper draws from the same data detailed in the study while using modern difference-in-differences methods. The first the difference-in-difference regression replicates @lang2013 while extending to propensity score matching methods to achieve a better balance between covariates between control and treatment states. Dynamic difference-in-differences methods are used for the data and found similar results as the preliminary difference-in-differences and Two-way Fixed Effects model.

### Mental Health Parity Act

In 1996, Congress pushed forward the MHPA to address rising concerns about mental health and how insurers treat those with mental health disorders. The MHPA aimed to provide equal treatment of regular physical and psychological services. With the MHPA, Congress banned group insurance plans that have a lower annual lifetime dollar cap for mental health services compared to regular services; however, there were no stipulations if insurance providers did not offer mental health services. Subsequent reports from the General Accounting Office (GAO) concerning the MHPA put into question whether or not the federal law had any effect. @GAO2000 While insurance companies were complying with the letter of the law, the law itself was only specific enough to require lifetime total costs between physical and mental treatment to be equal and did not prohibit other ways to limit utilization to insurance customers who had coverage through their insurance. For example, according to the GAO survey conducted after the MHPA was passed, employers bought group insurance plans that restricted the amount of in-hospital mental health-related stays. Following the aftermath of the MHPA, most states by 2002 enacted either their own more restrictive parity laws or accepted the federal law as is.[@samsha2023; @GAO2000]

### Parity laws:

@lang2013 introduced 4 categories that parity laws fall into, 2 of which are considered parity laws and the rest are parity related laws that are ineffective or not at parity. States that have implemented laws that require insurance companies to provide packages that include access to mental health services, as well as require that these services are equal to those that are provided for physical wellness, are considered to be treated by parity law. Parity law states have two types of parity law, one where the state forces the provider to include mental health coverage and the other must be compelled to offer the insurance buyer mental health care. The former is referred to just as a \`parity law\` while the latter is referred to as a \`mandated offering law\`. The non-parity laws are statutes that consist of either a \`mandated minimum benefit law\` where some sort of mental care must be included or a \`mandated if offered law\` where the mental health care is at parity *only* if the insurer provides it. 

## Data

The data set is constructed from multiple sources obtained through @lang2013. The data set is a sample of 15 years from 1990 to 2004 in 50 states including the District of Columbia. The matrix is a total of 765 observations. There are 6 main variables, the first of which is the main outcome variable logged crude suicide rate. I pull from the CDC WONDER database @centers for the compressed mortality statistics. Both queries included anything that was labeled under intentional self-harm fatalities in both ICD-9 and ICD-10. The same database also contained crude population statistics for each state in that given year. The Unemployment rate was obtained through The Current Population Survey. Bankruptcy statistics were obtained through the US bankruptcy court tables converted into Excel data and coded from the available population statistics. The percent of workers from large firms variable was created from the County Business Patterns Survey obtained through the US Census website.
...
Table 5 Introduces the simple difference-in-differences design that is explored in @lang2013. Both panels are averages weighted by state populations that denote pre and post-group cutoff between the two periods of 1990-1997 and 1998-2004. Standard errors are reported in parenthesis and in brackets are the number of observations that fall in this group. @lang2013 uses this specification to analyze the rest of his regressions. The main difference-in-difference results is replicated in both panels with a slightly higher Panel A result at 6.5% decrease in suicides. Panel B results at a similar 8%. Both are significant at the same levels in @lang2013 Table 3.

### My Difference in Difference

For the rest of this analysis, this paper will not be using the intervention timing assumption cut-off to conduct the rest of the regressions. All groups in the regressions after this point are accurate to the year they were implemented. That is to say, while @lang2013 had to handle treated states as if they were passed in 1998 for the difference-in-difference framework to function, this analysis is afforded the liberty of using the dynamic difference-in-difference. Additionally, the analysis has excluded things present in @lang2013 due to the author stating they had no impact on the main results. These include a lagged suicide rate timing control and dropping observations of states that passed a parity law in the middle of the year. Table 4 Column 2 also dropped observations so that only states that eventually become treated are left. I replicated this column with the data included and found no difference in the investigated effect.
...
Tables 7, 8, and 9 detail Difference-in-Differences regressions using two-way fixed effects. The same as previous models, variables are weighted by population at that specific state's point in time and additionally are clustered at the state level. Columns (1) and (2) detail the Difference in Difference models using Mahalanobis distance, switching between the regular and logged suicide rate. Columns (3) and (4) show models matched with a probit method. The only significant effect was found in column 4 with a -4% decrease associated with the post-treatment effect. Table 8 features GLM in columns (1) and (2) with the only significant effect belonging to column (2), finding the same coefficient as Table 7 Column (4). Columns (3) and (4) showcase models with Cardinality matching and find no significant effects. Table 9 Uses Coarse-Exact matching and Subclass matching to find the same coefficient in Column (4.) Overall every matching method that cut down on the observations or didn't use a logged crude rate did not retrieve significant effects.

### Dynamic treatment Effects

The second extension to @lang2013 is a dive into dynamic treatment effects using both @sun2021 and @callaway2021 to see if a roll-out design can capture the same effects found in the previous regressions. Here is an evaulation of the effect on each state's access to parity policy without worrying about introducing contamination through comparisons of past and future treated units. I will first introduce the @sun2021 results, compare them with @callaway2021 results, and  then complete the discussion with the @goodman-bacon2021 decomposition.
...
Table 11 contains the estimates for @callaway2021. All regressions use the parity law group timing discussed in the last section. Columns (1), (2), and (3) arrive at similar estimates of a 5% decrease in suicides, the first two using the universal base period while Column (3) uses a varying base period. In Column (4), I experimented with setting different anticipation periods and saw results at a significant 3% decrease in suicides with the anticipation setting at -1.

### Event Studies

Figure 1 and Figure 3 show the event studies from @sun2021 and @callaway2021 results respectively. The @sun2021 graph is overlaid with the three regressions from Table 10. The @callaway2021 event study is graphed from the first column regression in Table 11.Both event study tables show similarly significant results for at least 2 years after policy implementation.

In the @sun2021 event study, we can see pre-trends are apparent in the regressions. In Figure 1, some states experienced an significant effect of an increase in suicides 7 years before they were treated. When the event study moves the base period to 2 years before treatment, this effect is magnified and spreads to units that were treated 8 and 9 years in the future. This is evidence for a violation in the parallel trends assumptions and the @sun2021 regressions are not sound for conclusions. However, it\'s seen in Figure 3 that the @callaway2021 does not suffer from the same parallel trends violation and maintains significance two years after policy implementation.

### Goodman-Bacon

Figure 2 shows the results of the @goodman-bacon2021 decomposition graph breakdown of the weights involved in a Two-way Fixed Effects Regression on the log suicide rate without population weighting. From the graph, we can show that the treated vs untreated comparisons make up most of the weight on the ATT, only 30% of comparisons made consist of those "forbidden" comparisons of early vs later treated that can introduce contaminated results. So while the initial analysis in @lang2013 did primarly use the correct comparisons in their difference in difference, Figure 2 shows that by using dynamic difference in differences there is a tangible weighting bias reduction compared to previous estimates. 

## Conclusion

My findings concur with the of the seminal paper @lang2013 and estimate a negative 3-5% effect on suicides. Borrowing from @lang2013 back of the hand math, 29 states had a suicide rate of 10.24 and a total population of 160,221,455. A decrease of 3% to 5% would save between 492 to 820 lives nationally per year. Although the decrease in suicides could be attributed to the Mental Health Parity Act of 1996, it would be more apt to recognize it as a catalyst of change. The true mechanism behind these results were the laws that followed which enforced stricter guidelines for insurance firms. As explained in @harrison2002, the enacting of the MHPA created incentives for states to pass their version of parity law to circumvent federal oversight. Due to a provision in the law that dictates as long as a state fulfills the minimum requirements, it can retain the authority to regulate parity laws. This resulted in the wave of states implementing parity laws afterward.

One of the main limitations of this study revolves around the 1 in 10 rule for judging how many controls a model can handle before the model is in danger of overfitting. In the case of the regular two-way fixed effects, we have 3 main controls while we have 66 total state and year controls with a $n$ of 765 resulting in a rough ratio of 11 observations per control. Best practices taken from @harrell1985 dictate that this ratio is at the line where we would begin considering data-reduction methods or LASSO methods. For the newer methods, we have a total of 112 controls, for a cohort of 8 treatment years multiplied by the 14 years taken into consideration, dropping the first year of each cohort. This results in a ratio of roughly 7 observations per control, firmly below the rule of thumb. A combination of some cohorts having less than 30 observations causes logistic regression used by @callaway2021 and their doubly robust methods return blown-up standard errors and missing values.

Policy implications are straightforward. The data suggests stronger, more restrictive parity laws provide a pathway toward healthcare utilization that leads to suicide rate reduction.If full parity granted in 29 states resulted in a national 5% reduction in the suicide rate, it's worth pursuing politically. Future research using quality microdata would be a powerful way of working with the question of the effectiveness of mental health parity laws. One such study already exists in @drake2019 which looks at healthcare utilization after the 2008 Mental Health Parity and Addiction Equity Act. Additionally, papers like @ortega2023 which study Medicaid expansion while using dynamic differences-in-differences, are a great framework as the study calls to account for policy rollout rather than single implementation. Further research should turn their attention to the new 2024 Federal Parity rules that the Biden administration has implemented to combat non-quantitative treatment limitations that insurers use to inhibit utilization. Studies should be done not only to investigate effects of parity law on health care utilization but also outcomes like substance abuse statistics, attempted and completed suicides.[@ortega2023][@harrell1985]
